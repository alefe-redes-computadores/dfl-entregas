...
export function RouteOperations(){
  ...
  useEffect(()=>{if(!hydrated||settings.autoCloseCompletedRoutes===false)return;const today=dateKey(new Date());routes.filter(route=>route.status==='aberta'&&routeStartedAt(route)&&dateKey(routeDate(route) || new Date())<today).forEach(route=>{const linked=deliveries.filter(delivery=>delivery.route_id===route.id);if(linked.length&&linked.every(delivery=>delivery.completed)){const base=routeDate(route)||new Date();const end=new Date(base);end.setHours(23,59,0,0);void updateRoute(route.id,{status:'fechada',end_time:route.end_time||end.toISOString(),auto_closed_at:new Date().toISOString()})}})},[deliveries,hydrated,routes,settings.autoCloseCompletedRoutes,updateRoute]);
