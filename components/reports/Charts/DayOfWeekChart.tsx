    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-zinc-900/95 border border-zinc-700/50 rounded-xl p-3 backdrop-blur-sm shadow-lg">
          <p className="text-sm font-bold text-zinc-200 mb-2 border-b border-zinc-700/50 pb-1">{label}</p>
          <p className="text-sm text-pink-400 font-semibold">{`${payload[0].value} Entregas`}</p>
    ...
    <>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-bold text-zinc-200">Dias da Semana</h3>
        </div>
      ...
      <ResponsiveContainer width="100%" height={isExpanded ? '80%' : 260}>
        <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          ...
          <Bar yAxisId="left" dataKey="deliveries" name="Entregas" fill="#ec4899" radius={[4, 4, 0, 0]} animationDuration={1500} />
