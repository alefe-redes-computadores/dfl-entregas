...
import { ChevronDown, Bike, Wallet, CheckCircle2, RotateCcw, Timer, MapPin, Copy, User, UserRound, AlertTriangle, X, Trash2, Receipt, FileText } from 'lucide-react';
...
import { generateRouteMessages } from '@/lib/whatsapp';
...
export function RouteAccordion({ route, defaultOpen = false }: RouteAccordionProps) {
  ...
  const [isCopyMenuOpen, setIsCopyMenuOpen] = useState(false); // NOVO MODAL DE CÓPIA
  ...
  // Aciona a Geração das Mensagens (NOVA LÓGICA)
  const handleCopyMessage = async (msgType: 1 | 2) => {
    if (Capacitor.isNativePlatform()) await Haptics.impact({ style: ImpactStyle.Light });
    const storeAddr = storeSettings?.storeAddress || 'Patos de Minas, MG';
    
    // Gera as mensagens (A função devolve o array com as duas)
    const result = await generateRouteMessages(route, pendingDeliveries, storeAddr, getCustomerById);
  ...
  const executeCopyAction = async (msgType: 1 | 2, messages?: string[]) => {
    ...
    // Se não veio por parâmetro (veio do Modal Fuzzy), gera de novo
    const msgsToCopy = messages || (await generateRouteMessages(route, pendingDeliveries, storeAddr, getCustomerById)).messages;
    ...
    if (msgType === 1) {
      ...
      toast.success('Mensagem 2 copiada!', { description: 'Resumo e dinheiro prontos para o WhatsApp.' });
    }
    setIsCopyMenuOpen(false);
  };

  // Abrir o Maps Otimizado
  const handleOpenMaps = async () => {
    if (Capacitor.isNativePlatform()) await Haptics.impact({ style: ImpactStyle.Light });
    
    // Tenta usar o Link Manual salvo no cadastro se tiver, senão usa o endereço limpo
    const uniqueAddresses = Array.from(new Set(pendingDeliveries.map(d => {
      const cust = getCustomerById(d.customer_id);
      if (cust?.maps_link && cust.maps_link.includes('http')) return cust.maps_link;
  ...
  return (
    <div className={clsx("overflow-hidden rounded-[28px] border transition-all duration-300 relative", isNotStarted ? "bg-zinc-900/60 border-zinc-700/80" : isInProgress ? "bg-sky-900/10 border-sky-500/30" : "bg-emerald-500/10 border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.05)]")}>
      {totalDeliveries > 0 && !isCompleted && (<div className="absolute top-0 left-0 h-1 bg-zinc-800 w-full"><div className="h-full bg-emerald-500 transition-all duration-500" style={{ width: `${progressPercent}%` }} /></div>)}
      <button onClick={() => setIsOpen((prev) => !prev)} className="flex w-full items-center justify-between gap-3 p-4 pt-5 active:scale-[0.99] transition-transform">
        <div className="flex items-center gap-3">
          <div className={clsx('flex h-12 w-12 items-center justify-center rounded-full transition-colors shrink-0', isNotStarted ? 'bg-zinc-800 text-zinc-400' : isInProgress ? 'bg-sky-500/20 text-sky-400' : 'bg-emerald-500/20 text-emerald-500')}><Bike size={22} /></div>
          <div className="text-left flex flex-col">
            <div className="flex items-center gap-2"><p className={clsx("font-heading text-lg font-bold truncate max-w-[130px]", isCompleted ? "text-emerald-400" : "text-zinc-50")}>{route.name}</p>{isNotStarted && <span className="rounded-full bg-zinc-700 px-2 py-0.5 text-[9px] font-bold uppercase text-zinc-300">Montando</span>}{isInProgress && <span className="rounded-full bg-sky-500 px-2 py-0.5 text-[9px] font-bold uppercase text-white shadow-sm shadow-sky-500/30">Na Rua</span>}</div>
            <div className="flex items-center gap-1.5 mt-0.5 text-xs font-semibold text-zinc-400"><MotoIcon size={12} className={isInProgress ? 'text-sky-400' : isCompleted ? 'text-emerald-500' : 'text-zinc-500'} /><span className="truncate max-w-[120px]">{route.motoboy_name}</span></div>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {!isCompleted ? (<span className={clsx("rounded-full px-2.5 py-1 text-xs font-bold", pendingDeliveriesCount === 0 && totalDeliveries > 0 ? "bg-emerald-500 text-white" : isInProgress ? "bg-sky-500/20 text-sky-400" : "bg-zinc-800 text-zinc-400")}>{pendingDeliveriesCount === 0 && totalDeliveries > 0 ? 'Concluída!' : `${pendingDeliveriesCount} pendente${pendingDeliveriesCount !== 1 ? 's' : ''}`}</span>) : (<div className="flex items-center gap-2">{routeDuration && (<div className="flex items-center gap-1 bg-zinc-800/80 border border-zinc-700/50 px-2.5 py-1 rounded-full"><Timer size={12} className="text-zinc-400" /><span className="text-zinc-300 font-bold text-[10px]">{routeDuration}</span></div>)}<div className="flex items-center gap-1.5 bg-emerald-500/20 border border-emerald-500/30 px-2.5 py-1 rounded-full"><CheckCircle2 size={14} className="text-emerald-400" /><span className="text-emerald-400 font-bold text-[11px]">R$ {routeTotalValue.toFixed(2).replace('.',',')}</span></div></div>)}
      ...
      {isOpen && (
        <div className="flex flex-col gap-3 border-t border-zinc-800/80 p-4 pt-3 pb-6">
          {route.change_money > 0 && (<span className="self-start flex items-center gap-1.5 rounded-full bg-amber-500/10 px-2.5 py-1 text-xs font-bold text-amber-500"><Wallet size={13} /> Levar Troco: R$ {route.change_money.toFixed(2).replace('.', ',')}</span>)}
          {sortedDeliveries.length === 0 ? (<p className="py-4 text-center text-sm text-zinc-600">Nenhuma entrega nesta rota ainda.</p>) : (sortedDeliveries.map((delivery) => { const cust = getCustomerById(delivery.customer_id); const neighborhoodKey = cust?.neighborhood?.trim().toLowerCase(); const isNeighbor = neighborhoodKey ? (neighborhoodCounts[neighborhoodKey] > 1) : false; return (<DeliveryCard key={delivery.id} delivery={delivery} customer={cust} route={route} isNeighbor={isNeighbor} />); }))}
          
          <div className="mt-2 flex flex-col gap-2">
            {sortedDeliveries.length === 0 && isNotStarted && (<button onClick={handleDeleteEmptyRoute} className="flex w-full items-center justify-center gap-2 rounded-[20px] bg-red-500/10 border border-red-500/20 py-3.5 text-sm font-bold text-red-500 hover:bg-red-500/20 active:scale-95 transition-all"><Trash2 size={18} /> Excluir Rota Vazia</button>)}
            {sortedDeliveries.length > 0 && route.status === 'aberta' && (
              <div className="fixed bottom-24 left-0 right-0 z-40 mx-auto flex w-full max-w-[92%] items-center justify-between gap-3 rounded-[24px] border border-zinc-700/80 bg-zinc-900/95 px-4 py-3 backdrop-blur-xl shadow-2xl shadow-black/50">
                <button onClick={() => setIsCopyMenuOpen(true)} className="flex h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-zinc-800/80 text-sm font-semibold text-zinc-300 active:scale-95"><Copy size={18} className="text-emerald-500" /><span className="truncate">Copiar (WhatsApp)</span></button>
                <button onClick={handleOpenMaps} className="flex h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-indigo-600 text-sm font-bold text-white shadow-lg shadow-indigo-600/20 active:scale-95"><MapPin size={18} /><span className="truncate">Otimizar (Maps)</span></button>
              </div>
            )}
            {route.status === 'aberta' && totalDeliveries > 0 ? (!route.started_at ? (<button onClick={handleStartRoute} className="flex w-full items-center justify-center gap-2 rounded-[20px] bg-sky-500/10 border border-sky-500/20 py-3.5 text-sm font-bold text-sky-500 hover:bg-sky-500/20 active:scale-95"><Timer size={18} /> Iniciar Rota (Cronômetro)</button>) : (<button onClick={handleCloseRoute} className={clsx("flex w-full items-center justify-center gap-2 rounded-[20px] py-3.5 text-sm font-bold active:scale-95 transition-all", pendingDeliveriesCount === 0 && totalDeliveries > 0 ? "bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 animate-pulse" : "bg-zinc-800/80 text-zinc-300")}><CheckCircle2 size={18} className={pendingDeliveriesCount === 0 ? "text-white" : "text-emerald-500"} /> {pendingDeliveriesCount === 0 ? 'Tudo Entregue! Fechar Rota' : 'Finalizar Rota'}</button>)) : null}
            {route.status === 'fechada' && (<button onClick={() => setIsReopenModalOpen(true)} className="flex w-full items-center justify-center gap-2 rounded-[20px] bg-zinc-800/40 border border-zinc-700/50 py-3.5 text-sm font-semibold text-zinc-400 hover:bg-zinc-800 active:scale-95 mt-2"><RotateCcw size={16} /> Reabrir Rota (Correções)</button>)}
          </div>
        </div>
      )}
             ...
             <div className="flex flex-col gap-4">
                <button onClick={() => handleCopyMessage(1)} className="flex items-center gap-4 bg-zinc-900 border border-zinc-800 p-5 rounded-3xl active:scale-95 transition-all text-left">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-zinc-400 font-medium mt-1 leading-relaxed">Copia os endereços, IDs do iFood, observações e o link do Mapa otimizado.</span>
                  </div>
                </button>
                ...
                <button onClick={() => handleCopyMessage(2)} className="flex items-center gap-4 bg-zinc-900 border border-zinc-800 p-5 rounded-3xl active:scale-95 transition-all text-left">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-zinc-400 font-medium mt-1 leading-relaxed">Copia o resumo de bebidas da bag, status do Pix e valores do troco.</span>
                  </div>
                </button>
             </div>
          </div>
      ...
      {isReopenModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-2xl flex flex-col gap-4">
            ...
            <p className="text-sm text-zinc-300 leading-relaxed bg-amber-500/10 border border-amber-500/20 p-4 rounded-2xl">Se você precisa <strong className="text-amber-400">apenas consultar</strong> dados, não é necessário reabrir. As entregas já estão visíveis acima.</p>
