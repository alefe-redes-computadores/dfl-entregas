...
import { Share2, Banknote, CreditCard, QrCode, CupSoda, CheckCircle2, Pencil, Smartphone, Store, CheckCircle, ArrowUp, ArrowDown, AlertTriangle, User, UserRound, Star, Crown } from 'lucide-react';
...
export function DeliveryCard({ delivery, customer }: DeliveryCardProps) {
  ...
  // Busca o ícone minimalista do cliente
  let ClientIcon = User;
  ...
  async function handleToggleCompleted() {
    ...
    if (!delivery.completed) toast.success('Baixa Realizada! ✅', { duration: 1500 });
  }

  return (
    <div className={clsx(
        "overflow-hidden rounded-[20px] border border-zinc-800 bg-zinc-900/60 backdrop-blur-sm transition-all duration-300",
      ...
      <div className="flex flex-col p-3">
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="flex items-center gap-1.5">
            {isIfood ? (
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-500/10 text-red-500 text-[9px] font-bold uppercase tracking-wider"><Smartphone size={10} /> iFood</span>
            ) : (
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500 text-[9px] font-bold uppercase tracking-wider"><Store size={10} /> Loja</span>
            )}
            {isUrgent && <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-500 text-white text-[9px] font-black uppercase tracking-wider shadow-md shadow-red-500/30"><AlertTriangle size={10} /> Urgente</span>}
          </div>

          {delivery.completed && <span className="flex items-center gap-1 rounded-full bg-zinc-700 px-2 py-0.5 text-[10px] font-semibold text-zinc-300"><CheckCircle size={10} /> Entregue</span>}
        </div>
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex flex-col">
            <div className="flex items-baseline gap-2">
              {isIfood ? (
                <><p className="font-heading text-xl font-bold tracking-tight text-zinc-50">#{delivery.order_id}</p>{delivery.confirmation_code && <p className="rounded bg-zinc-800 px-1.5 py-0.5 text-[9px] font-mono tracking-wider text-zinc-400">Conf: {delivery.confirmation_code}</p>}</>
              ) : (
                <div className="flex items-center gap-1.5">
                  <ClientIcon size={14} className="text-emerald-500" />
                  <p className="font-heading text-lg font-bold tracking-tight text-zinc-50 truncate max-w-[200px]">{customer?.name || 'Sem Nome'}</p>
                </div>
              )}
            </div>
            
            <p className="mt-0.5 text-xs font-bold text-emerald-400">{isPrivacyMode ? 'R$ •••••' : `R$ ${delivery.value ? delivery.value.toFixed(2).replace('.', ',') : '0,00'}`}</p>
          </div>
        </div>

        {customer && isIfood && (
          <div className="mt-1.5 flex items-center gap-1.5">
            <ClientIcon size={12} className="text-red-400 shrink-0" />
            <p className="truncate text-xs font-semibold text-zinc-300">{customer.name}</p>
            {customer.neighborhood && <span className="shrink-0 rounded-full bg-zinc-800 px-1.5 py-0.5 text-[9px] font-medium text-zinc-400">{customer.neighborhood}</span>}
          </div>
        )}

        <p className={`truncate text-[11px] text-zinc-500 ${!isIfood ? 'mt-1.5' : 'mt-1'}`}>{delivery.address_string}</p>
        <div className="mt-2"><MiniMap address={delivery.address_string} mapsLink={delivery.maps_link} /></div>
      </div>

      {delivery.observation && (
        <div className="mx-3 mb-2 rounded-[12px] bg-zinc-800/60 px-2.5 py-1.5"><p className="text-[11px] text-zinc-400"><span className="font-semibold text-zinc-300">OBS: </span>{delivery.observation}</p></div>
      )}
      ...
      <div className="flex flex-col gap-2 border-t border-zinc-800/80 px-3 py-2.5">
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          <button onClick={handleToggleCompleted} className={clsx("flex-1 flex h-9 items-center justify-center gap-1.5 rounded-xl text-[13px] font-bold transition-all active:scale-95", delivery.completed ? "bg-zinc-800 text-zinc-400 border border-zinc-700" : "bg-emerald-500/10 text-emerald-500 border border-emerald-500/20")}>
            <CheckCircle2 size={14} />{delivery.completed ? 'Desfazer' : 'Baixa'}
          </button>
