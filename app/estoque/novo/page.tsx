...
import { formatStockQuantity, normalizeStockQuantityInput, parseStockQuantityInput, quantityInputHint } from '@/lib/stock-quantity';
import { StockCategoryPicker } from '@/components/stock/StockCategoryPicker';
import { StockPresentationEditor } from '@/components/stock/StockPresentationEditor';
import type { StockProduct, StockProductPresentation, StockSupplyUnit } from '@/types';

const numberValue = parseStockQuantityInput;
...
export default function NewStockProductPage() {
  ...
  const [current,setCurrent]=useState('0'); const [minimum,setMinimum]=useState('0'); const [ideal,setIdeal]=useState(''); const [leadTime,setLeadTime]=useState('2'); const [presentations,setPresentations]=useState<StockProductPresentation[]>([]); const [cost,setCost]=useState('R$ 0,00'); const [busy,setBusy]=useState(false);
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();if(!name.trim())return;if(invalidIdeal)return toast.error('O nível de reposição não pode ser menor que o nível de alerta.');setBusy(true);try{const now=new Date().toISOString();const product:StockProduct={id:`stock-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,name:name.trim(),category:category.trim()||undefined,unit,current_quantity:currentNumber,minimum_quantity:minimumNumber,ideal_quantity:target,lead_time_days:Math.max(0,numberValue(leadTime)),average_cost:moneyToNumber(cost)||undefined,presentations:presentations.filter(p=>p.label.trim()&&p.conversion_quantity>0).map(p=>({...p,label:p.label.trim()})),active:true,created_at:now,updated_at:now};await add(product);toast.success('Produto adicionado ao estoque.');router.replace('/estoque')}catch{toast.error('Não foi possível cadastrar o produto.');setBusy(false)}};
  return <div><PageHeader title="Novo produto" subtitle="Saldo, alerta e reposição" to="/estoque"/><form onSubmit={submit} className="space-y-5 pb-10">
    <TextField label="Produto*" value={name} set={setName} placeholder="Ex.: Coca-Cola 2L" required/><div className="block text-xs font-bold text-zinc-400"><p>Categoria</p><StockCategoryPicker value={category} onChange={setCategory}/><p className="mt-2 text-[9px] font-normal leading-relaxed text-zinc-600">Classificação operacional do produto. Pode ser alterada depois sem reescrever o histórico de movimentações.</p></div>
    ...
    <section className="rounded-[24px] border border-zinc-800 bg-zinc-900/35 p-4"><h2 className="font-heading text-sm font-black text-zinc-200">Controle de quantidade</h2><p className="mt-1 text-[10px] leading-relaxed text-zinc-500">O alerta avisa que está acabando. A reposição define até onde completar a compra.</p><div className="mt-4 grid grid-cols-1 gap-3 min-[360px]:grid-cols-2"><NumberField label="Quantidade atual" hint={quantityInputHint(unit)} value={current} set={setCurrent}/><NumberField label="Avisar quando chegar em" hint={quantityInputHint(unit)} value={minimum} set={setMinimum}/><NumberField label="Repor até" hint={quantityInputHint(unit)} value={ideal} set={setIdeal} placeholder={minimum||'0'}/><NumberField label="Prazo de reposição" hint="Dias entre comprar e receber" value={leadTime} set={setLeadTime} placeholder="2"/><label className="min-w-0 text-xs font-bold text-zinc-400 min-[360px]:col-span-2">Custo por unidade<span className="mt-1 block text-[9px] font-normal text-zinc-600">Valor médio atual</span><input inputMode="numeric" value={cost} onChange={e=>setCost(formatBRLCents(e.target.value))} className={inputClass}/></label></div>
    ...
    <StockPresentationEditor baseUnit={unit} value={presentations} onChange={setPresentations}/>
