...
export default function NewStockProductPage() {
  ...
  const [current,setCurrent]=useState('0'); const [minimum,setMinimum]=useState('0'); const [ideal,setIdeal]=useState(''); const [leadTime,setLeadTime]=useState('2'); const [cost,setCost]=useState('R$ 0,00'); const [busy,setBusy]=useState(false);
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();if(!name.trim())return;if(invalidIdeal)return toast.error('O nível de reposição não pode ser menor que o nível de alerta.');setBusy(true);try{const now=new Date().toISOString();const product:StockProduct={id:`stock-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,name:name.trim(),category:category.trim()||undefined,unit,current_quantity:currentNumber,minimum_quantity:minimumNumber,ideal_quantity:target,lead_time_days:Math.max(0,numberValue(leadTime)),average_cost:moneyToNumber(cost)||undefined,active:true,created_at:now,updated_at:now};await add(product);toast.success('Produto adicionado ao estoque.');router.replace('/estoque')}catch{toast.error('Não foi possível cadastrar o produto.');setBusy(false)}};
