...
export default function StockPage() {
  ...
  const [planningTab,setPlanningTab]=useState<'dia'|'montar'>(()=>typeof window==='undefined'?'dia':((localStorage.getItem('dfl-stock-planning-tab') as 'dia'|'montar')||'dia'));
  ...
  useEffect(()=>{localStorage.setItem('dfl-stock-planning-tab',planningTab)},[planningTab]);
  useEffect(()=>{
    if(restoredScroll.current||!products.length)return;
    restoredScroll.current=true;
    const y=Number(sessionStorage.getItem('dfl-stock-scroll')||0);
    if(y<=0)return;
    let tries=0;
    const restore=()=>{
      window.scrollTo({top:y,behavior:'auto'});
      tries+=1;
      if(tries<6&&Math.abs(window.scrollY-y)>8)setTimeout(restore,60*tries);
    };
    requestAnimationFrame(restore);
  },[products.length]);
  useEffect(()=>{const save=()=>sessionStorage.setItem('dfl-stock-scroll',String(window.scrollY));window.addEventListener('scroll',save,{passive:true});return()=>{save();window.removeEventListener('scroll',save)}},[]);
  const rememberAndPush=(href:string)=>{sessionStorage.setItem('dfl-stock-scroll',String(window.scrollY));router.push(href)};
  const leaveStock=()=>{sessionStorage.removeItem('dfl-stock-scroll');router.replace('/loja')};
  ...
  return <div className="dfl-page">
    <header className="flex items-center justify-between"><div className="flex items-center gap-3"><button onClick={leaveStock} className="grid h-12 w-12 place-items-center rounded-full border border-zinc-800 bg-zinc-900 text-zinc-400"><ChevronLeft size={21}/></button><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-emerald-400">Controle físico</p><h1 className="font-heading text-2xl font-black text-zinc-100">Estoque</h1></div></div></header>
    ...
    <section className="grid grid-cols-2 gap-2"><Shortcut icon={ClipboardCheck} label="Contagem" color="text-sky-400" onClick={()=>rememberAndPush('/estoque/contagem')}/><Shortcut icon={ListChecks} label="Comprar" color="text-amber-400" onClick={()=>rememberAndPush('/estoque/compras')}/><Shortcut icon={BarChart3} label="Relatórios" color="text-emerald-400" onClick={()=>rememberAndPush('/estoque/relatorios')}/><Shortcut icon={Archive} label="Arquivados" color="text-zinc-400" onClick={()=>rememberAndPush('/estoque/arquivados')}/><Shortcut icon={Store} label="Fornecedores" color="text-orange-400" onClick={()=>rememberAndPush('/estoque/fornecedores')}/><Shortcut icon={BadgeDollarSign} label="Preços" color="text-lime-400" onClick={()=>rememberAndPush('/estoque/precos')}/><Shortcut icon={WandSparkles} label="Catálogo" color="text-violet-400" onClick={()=>rememberAndPush('/estoque/catalogo')}/></section>
    ...
    {suggested.length>0&&<section className="rounded-[22px] border border-amber-500/25 bg-amber-500/[.055] p-4">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        {suggested.slice(0,4).map(p=>{const rec=recommendationMap.get(p.id)!;return <button type="button" key={p.id} onClick={()=>rememberAndPush(`/estoque/detalhes?id=${p.id}&from=estoque`)} className="rounded-xl bg-zinc-950/50 px-3 py-2 text-left transition active:scale-[.98] active:bg-zinc-900">
    ...
    {(daily.length>0||suggested.length>0)&&<section className="rounded-[22px] border border-emerald-500/15 bg-emerald-500/[.035] p-4">
      <div className="flex items-center gap-2"><CalendarDays size={16} className="text-emerald-400"/><div><p className="text-xs font-black text-zinc-100">Planejamento de compra</p><p className="text-[9px] text-zinc-500">Veja a necessidade de hoje ou avance para montar a compra.</p></div></div>
      <div className="mt-3 grid grid-cols-2 rounded-xl bg-zinc-950/55 p-1">
        <button type="button" onClick={()=>setPlanningTab('dia')} className={`h-9 rounded-lg text-[9px] font-black transition ${planningTab==='dia'?'bg-emerald-500/12 text-emerald-400':'text-zinc-600'}`}>Compra do dia</button>
        <button type="button" onClick={()=>setPlanningTab('montar')} className={`h-9 rounded-lg text-[9px] font-black transition ${planningTab==='montar'?'bg-amber-500/12 text-amber-400':'text-zinc-600'}`}>Montar compra</button>
      </div>
      {planningTab==='dia'?<>
        <p className="mt-3 text-[9px] leading-relaxed text-zinc-500">Sugestão contextual pelo consumo deste dia da semana; sem amostra, mantém sua meta.</p>
        {daily.length>0?<><div className="mt-3 grid grid-cols-2 gap-2">{daily.slice(0,4).map(({p,signal})=><button key={p.id} onClick={()=>rememberAndPush(`/estoque/detalhes?id=${p.id}&from=estoque`)} className="rounded-xl bg-zinc-950/45 p-3 text-left"><p className="truncate text-[10px] font-black text-zinc-300">{p.name}</p><p className="mt-1 text-[9px] font-black text-emerald-400">{humanPurchasePlan({purchaseQuantity:signal.commercialPlan.purchaseQuantity,baseQuantity:signal.commercialPlan.baseQuantity,baseUnit:p.unit,presentation:signal.commercialPlan.presentation})} sugerido</p><p className="mt-1 text-[8px] text-zinc-600">{signal.usesWeekday?`${signal.weekdaySamples} dias equivalentes · confiança ${signal.confidence}`:'Meta configurada · histórico insuficiente'}</p></button>)}</div>{daily.length>4&&<p className="mt-2 text-[9px] font-bold text-zinc-600">+ {daily.length-4} outros produtos</p>}</>:<p className="mt-3 rounded-xl bg-zinc-950/35 p-3 text-[10px] text-zinc-600">Nenhuma necessidade para hoje.</p>}
      </>:<div className="mt-3 rounded-2xl border border-amber-500/15 bg-amber-500/[.04] p-3">
        <p className="text-[10px] font-black text-zinc-200">{suggested.length?`${suggested.length} ${suggested.length===1?'produto pronto':'produtos prontos'} para revisar`:'Estoque sem reposição pendente'}</p>
        <p className="mt-1 text-[9px] leading-relaxed text-zinc-500">A tela de compra aplica apresentações, quantidades já comprometidas e permite revisar os itens antes de criar a compra.</p>
        <button type="button" onClick={()=>rememberAndPush('/estoque/compras')} className="mt-3 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-amber-500 text-[10px] font-black text-zinc-950"><ListChecks size={15}/>Abrir montagem da compra</button>
      </div>}
    </section>}
