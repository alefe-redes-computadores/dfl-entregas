...
const dateTime = (value?: string) => value ? new Date(value).toLocaleString('pt-BR') : 'Não registrado';
...
function DeliveryDetailsContent() {
  ...
  if (!delivery) return <div className="flex min-h-[55vh] flex-col items-center justify-center gap-4 text-center"><AlertTriangle className="text-amber-400" size={36}/><div><h1 className="font-heading text-xl font-bold text-zinc-100">Entrega não encontrada</h1><p className="mt-1 text-sm text-zinc-500">Ela pode ter sido removida ou ainda não sincronizou.</p></div><button onClick={()=>router.replace('/entregas')} className="rounded-xl bg-zinc-800 px-4 py-3 text-sm font-bold text-zinc-200">Voltar às entregas</button></div>;

  const name = customer?.name || delivery.customer_name || 'Cliente não informado';
  const phone = delivery.phone || customer?.phone;
  const mapsUrl = delivery.maps_link || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(delivery.address_string || 'Patos de Minas, MG')}`;
  const paymentIcon = delivery.payment_method === 'dinheiro' ? Banknote : delivery.payment_method === 'pix' ? QrCode : CreditCard;
  ...
  return <div className="flex flex-col gap-5 pb-28 animate-in fade-in duration-300">
    <header className="flex items-center gap-3"><button onClick={()=>router.replace('/entregas')} className="flex h-10 w-10 items-center justify-center rounded-full bg-zinc-900 text-zinc-400"><ChevronLeft size={21}/></button><div className="min-w-0 flex-1"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-amber-500">Ficha da entrega</p><h1 className="truncate font-heading text-xl font-bold text-zinc-50">{name}</h1></div><button onClick={()=>router.push(`/entregas/editar?id=${delivery.id}`)} className="flex h-10 items-center gap-2 rounded-xl bg-amber-500 px-3 text-xs font-black text-zinc-950"><Edit3 size={15}/>Editar</button></header>
    ...
    <section className="overflow-hidden rounded-[24px] border border-zinc-800 bg-zinc-900/40"><InfoRow icon={Bike} label="Rota" value={route ? `${route.name} • ${route.motoboy_name}` : 'Rota não encontrada'}/><InfoRow icon={Clock3} label="Criada em" value={dateTime(delivery.created_at || delivery.createdAt)}/><InfoRow icon={CheckCircle2} label="Conclusão" value={delivery.completed ? dateTime(delivery.completed_at) : 'Ainda pendente'}/>{delivery.confirmation_code&&<InfoRow icon={Smartphone} label="Código de confirmação" value={delivery.confirmation_code}/>}</section>
  </div>;
}

function InfoRow({icon:Icon,label,value}:{icon:typeof MapPin;label:string;value:string}) { return <div className="flex items-start gap-3 border-b border-zinc-800/80 p-4 last:border-0"><div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-zinc-800 text-zinc-400"><Icon size={17}/></div><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">{label}</p><p className="mt-1 break-words text-sm font-semibold leading-relaxed text-zinc-200">{value}</p></div></div>; }

export default function DeliveryDetailsPage(){return <Suspense fallback={<div className="p-10 text-center text-zinc-500">Carregando entrega...</div>}><DeliveryDetailsContent/></Suspense>;}
