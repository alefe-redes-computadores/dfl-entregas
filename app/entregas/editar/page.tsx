          ...
          {origin === 'ifood' && (
            <div className="rounded-2xl border border-zinc-800 bg-zinc-950/35 p-3">
              <button
                type="button"
                onClick={()=>{
                  setHasIfoodSubsidy(v=>!v);
                  if(hasIfoodSubsidy)setIfoodSubsidy('');
                }}
                className="flex w-full items-center justify-between gap-3 text-left"
              >
                <span>
                  <span className="block text-[11px] font-black text-zinc-300">
                    Cupom / subsídio do iFood
                  </span>
                  <span className="mt-0.5 block text-[9px] text-zinc-600">
                    O motoboy cobra somente o valor do cliente.
                  </span>
                </span>
                <span className={`shrink-0 rounded-full px-2 py-1 text-[9px] font-black ${hasIfoodSubsidy?'bg-emerald-500/10 text-emerald-400':'bg-zinc-800 text-zinc-500'}`}>
                  {hasIfoodSubsidy?'Ativo':'Adicionar'}
                </span>
              </button>
                  ...
                  <div className="flex min-w-0 flex-col justify-center rounded-xl border border-emerald-500/20 bg-emerald-500/[.055] p-2.5">
                    ...
                    <p className="mt-1 truncate text-base font-black text-zinc-100">
                      R$ {(
                        ...
                        Number(ifoodSubsidy.replace(/\./g, '').replace(',', '.'))
