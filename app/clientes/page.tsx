        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-3xl p-5 shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={closeEditModal} className="text-zinc-500 hover:text-zinc-300"><X size={24} /></button>
            </div>
            ...
            <form onSubmit={handleSaveCustomer} className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-4 bg-zinc-900/50 p-4 rounded-xl border border-zinc-800">
                <button type="button" onClick={() => setIsAvatarModalOpen(true)} className="relative h-14 w-14 rounded-full bg-zinc-950 border border-zinc-800 flex items-center justify-center hover:bg-zinc-800 group overflow-hidden shrink-0">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-zinc-100">Foto do Cliente</span>
                  <span className="text-xs text-zinc-500">Clique para alterar o ícone.</span>
                </div>
              </div>

              <div className="flex gap-2 p-1 bg-zinc-900 rounded-2xl border border-zinc-800">
                <button type="button" onClick={() => setEditOrigin('ifood')} className={`flex-1 flex items-center justify-center h-10 rounded-xl font-bold text-sm ${editOrigin === 'ifood' ? 'bg-red-500 text-white' : 'text-zinc-500'}`}>iFood</button>
              ...
              {CUSTOMER_AVATARS.map(opt => (
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-3 rounded-2xl border transition-all ${
                    editAvatar === opt.id ? `bg-zinc-800 border-zinc-600 shadow-md` : `bg-zinc-950 border-zinc-800/80 hover:bg-zinc-800`
                  }`}
                >
                  <div className={`h-10 w-10 rounded-full flex items-center justify-center ${opt.bg}`}>
                    <RenderCustomerAvatar id={opt.id} size={20} />
                  </div>
                </button>
              ))}
            </div>
