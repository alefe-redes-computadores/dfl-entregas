...
function CustomerDetailsContent(){
  ...
  if(!customer)return <div className="py-20 text-center"><p className="font-bold text-zinc-200">Cliente não encontrado</p><button onClick={()=>router.replace('/clientes')} className="mt-4 text-sm font-bold text-emerald-400">Voltar aos clientes</button></div>;
  ...
  return createdAt
    ...
    : 'Data não registrada';
})()}</p></div><div className="text-right"><p className="text-sm font-black text-emerald-400">{money(deliveryCustomerCharge(delivery))}</p><span className={`mt-2 inline-block rounded-md px-2 py-0.5 text-[9px] font-bold ${delivery.completed?'bg-emerald-500/10 text-emerald-400':'bg-amber-500/10 text-amber-400'}`}>{delivery.completed?'Concluído':'Pendente'}</span><ChevronRight size={15} className="ml-auto mt-2 text-zinc-600"/></div></div></button>})}{stats.deliveries.length===0&&<div className="rounded-3xl border border-dashed border-zinc-800 py-12 text-center"><PackageOpen className="mx-auto text-zinc-700"/><p className="mt-3 text-sm text-zinc-500">Nenhum pedido vinculado.</p></div>}</div></section>
  </div>;
}
