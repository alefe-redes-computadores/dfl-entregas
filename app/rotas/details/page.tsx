...
export default function RouteDetailsPage() {
  const router = useRouter(); const params = useSearchParams(); const id = params.get('id'); const date = params.get('date') || ''; const routesReturn = date ? `/rotas?date=${encodeURIComponent(date)}` : '/rotas'; const dateSuffix = date ? `&date=${encodeURIComponent(date)}` : '';
  ...
  if (!route) return <div className="py-20 text-center"><p className="text-zinc-400">Rota não encontrada.</p><button onClick={()=>router.replace(routesReturn)} className="mt-4 text-emerald-400">Voltar às rotas</button></div>;
  ...
  return <div className="flex flex-col gap-5 pb-28">
    <div className="flex items-center gap-3"><button onClick={()=>router.replace(routesReturn)} className="flex h-10 w-10 items-center justify-center rounded-full bg-zinc-900"><ArrowLeft size={20}/></button><div className="min-w-0 flex-1"><p className="text-xs text-zinc-500">Detalhes da rota</p><h1 className="truncate font-heading text-xl font-bold">{route.name}</h1></div><button onClick={()=>router.push(`/rotas/editar?id=${route.id}${dateSuffix}`)} className="flex h-10 w-10 items-center justify-center rounded-full border border-zinc-800 bg-zinc-900"><Edit3 size={17}/></button></div>
    ...
    <div><h2 className="mb-3 font-bold">Paradas</h2><div className="space-y-2">{deliveries.map((delivery,index)=><button key={delivery.id} onClick={()=>router.push(`/entregas/details?id=${delivery.id}${dateSuffix}`)} className="flex w-full items-center gap-3 rounded-2xl border border-zinc-800 bg-zinc-900/45 p-3 text-left"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-zinc-800 text-xs font-bold">{index+1}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{delivery.customer_name || `Pedido ${delivery.order_id || ''}`}</p><p className="truncate text-xs text-zinc-500"><MapPin size={11} className="mr-1 inline"/>{delivery.address_string}</p></div>{delivery.completed?<CheckCircle2 size={18} className="text-emerald-400"/>:<Clock3 size={18} className="text-zinc-600"/>}</button>)}{deliveries.length===0&&<p className="rounded-2xl border border-dashed border-zinc-800 py-8 text-center text-sm text-zinc-500">Nenhuma entrega vinculada.</p>}</div></div>
